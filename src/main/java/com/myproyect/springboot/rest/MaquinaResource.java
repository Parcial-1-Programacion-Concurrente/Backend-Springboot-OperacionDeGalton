...
public class MaquinaResource {
    ...
    public ResponseEntity<MaquinaDTO> getMaquina(@PathVariable final Integer id) {
        return ResponseEntity.ok(maquinaService.get(id));
    }

    @PostMapping
    @ApiResponse(responseCode = "201", description = "Create a new maquina")
    public ResponseEntity<Integer> createMaquina(@RequestBody final MaquinaDTO maquinaDTO) {
        final Integer createdId = maquinaService.create(maquinaDTO);
        return new ResponseEntity<>(createdId, HttpStatus.CREATED);
    }

    @PutMapping("/{id}")
    @ApiResponse(responseCode = "200", description = "Update an existing maquina")
    public ResponseEntity<Void> updateMaquina(@PathVariable final Integer id,
